...
{ "name": "rdp-client-2026", "version": "14.5.0", "description": "Consola de Administración para RDP Client 2026", "main": "index.js", "scripts": { "start": "node index.js" }, "dependencies": { "express": "^4.19.2" } }
EOF
...
while true; do
    ...
    if grep -q "DISCONNECT_BY_USER" "$LOG_FILE"; then
        ...
        tail -n 20 "$LOG_FILE"
        echo -e "${YELLOW}-----------------------------------------------------------------${NC}"
        echo -e "\n  ${YELLOW}Causas comunes:${NC} Usuario/contraseña, DNS, o error de seguridad."
